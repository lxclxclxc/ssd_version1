...
*****************************************************************************************************************************/
#include "ssd.h"
int printf_every_existing_unfinishedsubrequests_in_request(struct ssd_info *ssd)
{
    ...
    if(temp_num_request == 0){
        ...
        while(temp_request != NULL){
            ...
            // here is checking subrequests status staying in requests 
            sub = temp_request->subs;
            while(sub != NULL){
                ...
                {
                    printf("sub status sub %lld,  channle: %d, chip: %d, die %d, plane %d, lpn: %d, ppn: %d  operation: %d,size: %d,   ////sub_request status:: current_state: %d, current_time: %llu, next_state: %d, next_state_predict_time: %llu, state(only for write): %d\n",sub, sub->location->channel, sub->location->chip, sub->location->die, sub->location->plane,  sub->lpn, sub->ppn,sub->operation, sub->size, sub->current_state, sub->current_time,sub->next_state, sub->next_state_predict_time, sub->state) ;
                }
...
void printf_channel_chip_timestamp(struct ssd_info *ssd)
{
    int i,j;
...
int judge_wrong_time_stamp(struct ssd_info *ssd)
{
    int i, j;

    for (i = 0; i < ssd->parameter->channel_number; i++) {
//        if( (ssd->channel_head[i].next_state_predict_time < ssd->current_time) &&  (ssd->channel_head[i].next_state_predict_time > ssd->channel_head[i].current_time) ){
//            printf("channel %d timestamp wrong\n", i);
//            printf("ssd currenttime is %lld\n", ssd->current_time);
//
//            printf("channel[%d].next_state is 0x%x,ssd->channel_head[%d].next_state_predict_time is %lld, channel[%d].current_state is 0x%x,ssd->channel_head[%d].current_time is %lld\n",i,ssd->channel_head[i].next_state,i,ssd->channel_head[i].next_state_predict_time  ,i,ssd->channel_head[i].current_state,i,ssd->channel_head[i].current_time  );
//            while(1)
//            {}
//        }
        ...
        for (j = 0; j < ssd->parameter->chip_channel[i]; j++) {
            if( (ssd->channel_head[i].chip_head[j].next_state_predict_time < ssd->current_time) &&  (ssd->channel_head[i].chip_head[j].next_state_predict_time > ssd->channel_head[i].chip_head[j].current_time) ){
                printf("chip %d timestamp wrong\n", j);
                printf("ssd currenttime is %lld\n", ssd->current_time);
...
//
//            int temp_write_number = statistic_write_request_number(ssd);
//            printf("ssd->current_time is %lld, now write request number is %d\n",ssd->current_time, temp_write_number );
//                for (i = 0; i < ssd->parameter->channel_number; i++) {
//
//        printf("channel[%d].next_state is 0x%x,ssd->channel_head[%d].next_state_predict_time is %lld, channel[%d].current_state is 0x%x,ssd->channel_head[%d].current_time is %lld\n",i,ssd->channel_head[i].next_state,i,ssd->channel_head[i].next_state_predict_time  ,i,ssd->channel_head[i].current_state,i,ssd->channel_head[i].current_time  );
//
...
//    }
        ...
        for (j = 0; j < ssd->parameter->chip_channel[i]; j++) {
            ...
            printf_every_existing_unfinishedsubrequests_in_request(ssd);
...
//            for(temp_i = 0; temp_i < ssd->request_queue_length; temp_i ++){
//
//                if( (request1->operation != 1 ) &&(request1 != NULL) ) {
//                    request1 = request1->next_node;
//
//                }else{
//
...
//
//                    ssd->interference_stuffed_length ++;
//                    printf("deal inner subrequest, current_time %lld, temp_lba = %lld, interference_stuffed_length is %d \n", ssd->current_time, ssd->temp_lba, ssd->interference_stuffed_length);
//
//                    for(temp_i = 0; temp_i < ssd->parameter->channel_number; temp_i ++ ){
//
//                        temp_gc_node = ssd->channel_head[i].gc_command;
