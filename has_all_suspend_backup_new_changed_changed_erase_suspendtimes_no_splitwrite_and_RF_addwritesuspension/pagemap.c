...
struct local *find_location(struct ssd_info *ssd, unsigned int ppn) {
    ...
    //printf("page_block is %d, block_plane is %d, plane_die is %d, die_chip is %d, chip_channel is %d\n", ssd->parameter->page_block, ssd->parameter->block_plane, ssd->parameter->plane_die, ssd->parameter->die_chip, ssd->parameter->chip_channel[0]);
    /*******************************************************************************
...
******************************************************************************/
unsigned int
find_ppn(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane,
...
struct ssd_info *pre_process_page(struct ssd_info *ssd) {
    ...
    int64_t time;
//lxcprogram_gc
    unsigned int l, temp_chip_channel ;
    ...
    //lxcprogram_gc , record the total pages number
ssd->original_total_pages_number = (ssd->parameter->chip_num * ssd->parameter->die_chip * ssd->parameter->plane_die *ssd->parameter->block_plane * ssd->parameter->page_block);
ssd->after_pre_process_pages_number = ssd->original_total_pages_number * (1 - ssd->parameter->overprovide);
    /*计算出这个ssd的最大逻辑扇区号*/
    ...
    while (fgets(buffer_request, 200, ssd->tracefile)) {
        ...
        if (ope == 1)                                                      /*这里只是读请求的预处理，需要提前将相应位置的信息进行相应修改*/
        {
            while (add_size < size) {
                ...
                sub_size = ssd->parameter->subpage_page - (lsn % ssd->parameter->subpage_page);//此处看似不合理，其实只在开头和结尾会出现少于2K的情况，因为在函数后面有专门的每次给sub_size和lsn都分别加上这个sub_size了。//lxc
                if (add_size + sub_size >=
                    size)                             /*只有当一个请求的大小小于一个page的大小时或者是处理一个请求的最后一个page时会出现这种情况*/
                {
                ...
                if ((sub_size > ssd->parameter->subpage_page) ||
                    (add_size > size))/*当预处理一个子大小时，这个大小大于一个page或是已经处理的大小大于size就报错*/
                {
				...
				********************************************************************************************************/
                ...
                if (ssd->dram->map->map_entry[lpn].state == 0)                 /*状态为0的情况*/
                {
					...
					***************************************************************/
                    ...
                    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].free_state = (
        ...
        for (l = 0; l < temp_chip_channel; l++){
            for (j = 0; j < ssd->parameter->die_chip; j++){
                for (k = 0; k < ssd->parameter->plane_die; k++){
                    ...
                    ssd->after_pre_processe_and_aged_pages_number +=  ssd->channel_head[i].chip_head[l].die_head[j].plane_head[k].free_page;
...
unsigned int get_ppn_for_pre_process(struct ssd_info *ssd, unsigned int lsn) {
    ...
    {
        if (ssd->parameter->dynamic_allocation ==
            0)                      /*表示全动态方式下，也就是channel，chip，die，plane，block等都是动态分配*/
        {
            channel = ssd->token;
        ...
        } else if (ssd->parameter->dynamic_allocation ==
                   1)                 /*表示半动态方式，channel静态给出，package，die，plane动态分配*/
        {
	...
	******************************************************************************/
#ifdef DEBUG
    printf("and get channel %d,chip %d, die %d, plane %d\n", channel, chip, die, plane);
#endif
...
//lxcprogram_gc 
	...
	**************************************************************************************/
    ...
    if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].last_write_page >
            ssd->parameter->page_block) {
        ...
        0)                                       /*this is the first logical page*/
    {
        if (ssd->dram->map->map_entry[lpn].pn != 0) {
            printf("new logical, Error in get_ppn()\n");
        }
    ...
    } else                                                                            /*这个逻辑页进行了更新，需要将原来的页置为失效*/
    {
        ppn = ssd->dram->map->map_entry[lpn].pn;
        location = find_location(ssd, ppn);
        if (ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn !=
            lpn) {
            printf("\nchange logical,Error in get_ppn(), maptable.ppn is %d, page saved ppn is %d, wrong location channel %d, chip %d, die %d, plane %d, block %d, page %d\n", ppn,ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn, location->channel, location->chip, location->die, location->plane, location->block, location->page);
        }
    ...
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].page_head[page].free_state = (
            (~(sub->state)) & full_page);
    ...
    if (ssd->parameter->active_write ==
        0)                                            /*如果没有主动策略，只采用gc_hard_threshold，并且无法中断GC过程*/
    {
                                                                                       /*如果plane中的free_page的数目少于gc_hard_threshold所设定的阈值就产生gc操作*/
        threshold_page = (ssd->parameter->page_block) * (ssd->parameter->block_plane) * (ssd->parameter->gc_hard_threshold);
#ifdef DEBUG

        printf("opps, must be here!! for no active_write.now is in no active_write strategy\n");
#endif

//#ifdef DEBUGONLYGC
        printf("when normal write everytime use one page  will report plane's free_page as follow: channel %d, chip %d, die %d, plane %d, block %d, page %d, free_page %d\n", channel, chip, die, plane, block, page,  ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page);
//#endif


if(ssd->channel_head[channel].gc_command != NULL){

printf("lxc, here will stop to move ahead,will return. in channel GC, even cost one page\n");
    return ssd;
}

        if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page <
          ...
          //0) {
            ...
            //lxcprogram
            gc_node->copy_page_number = -1;//for read_transfer_write part.
            gc_node->copy_valid_number = -1; //for read_transfer_write part.
            gc_node->pos_in_copy = -1; //no use now
            gc_node->read = 1;//for read_transfer_write's  read phase.
            gc_node->write = 0;//for read_transfer_write's  write phase.
            gc_node->erase = -1;//for gc's  erase phase
            gc_node->victim_block_number = -1; // destinational plane's viction block for gc
            gc_node->gc_read_inner = -1;//for read_transfer_write's  read phase's sub_phase.
            gc_node->gc_write_inner = -1;//for read_transfer_write's  write phase's sub_phase.
            gc_node->read_write_end = -1;//for read_transfer_write's  write phase's sub_phase.
            gc_node->victim_block_freepage_statistic = -1;//for the victim block's free page number.
            gc_node->last_page_in_victim_deal = -1; //for the last page sign.
            gc_node->chip_next_predict_time_diff = 0;
            ssd->channel_head[channel].gc_command = gc_node;
            ssd->gc_request++;
            //ssd->debug_sign = 1;// begin to printf

        printf("GC will happen at once!\n");
#ifdef DEBUGONLYGC
        printf("GC happen now!\n");
        printf_gc_node_information (gc_node);
            ...
            printf("normal get_ppn 's gc_node content,%x\n", ssd->gc_request);
            printf_ssd_gc_node(ssd);
#endif

        }//end of free page < threshold
    }
...
******************************************************************************************/
unsigned int
get_ppn_for_gc(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane) {
    unsigned int ppn;
    ...
    ssd->write_flash_count++;
#ifdef DEBUGONLYGC
    printf("when gc get the new ppn, channel %d, chip %d, die %d, plane %d, block %d\n", channel, chip, die, plane, block);
#endif
    return ppn;
...
Status
erase_operation(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane,
                unsigned int block) {
    unsigned int i = 0;
#ifdef DEBUG
    printf("now erase channel %d, chip %d, die %d, plane %d, block %d \n", channel, chip, die, plane, block);
#endif
...
***************************************************************************************/
Status
erase_planes(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die1, unsigned int plane1,
    ...
    if ((ssd->channel_head[channel].chip_head[chip].die_head[die1].plane_head[plane1].erase_node == NULL) ||
        ((command != INTERLEAVE_TWO_PLANE) && (command != INTERLEAVE) && (command != TWO_PLANE) &&
         (command != NORMAL)))     /*如果没有擦除操作，或者command不对，返回错误*/
    {
        return ERROR;
    }
	...
	*************************************************************************************************************/
    ...
    if (command == INTERLEAVE_TWO_PLANE)                                       /*高级命令INTERLEAVE_TWO_PLANE的处理*/
    {
        ...
        ssd->channel_head[channel].next_state_predict_time =
                ssd->current_time + 18 * ssd->parameter->time_characteristics.tWC +
                ssd->parameter->time_characteristics.tWB;
        ssd->channel_head[channel].chip_head[chip].next_state_predict_time =
                ssd->channel_head[channel].next_state_predict_time - 9 * ssd->parameter->time_characteristics.tWC +
    ...
    } else if (command == INTERLEAVE)                                          /*高级命令INTERLEAVE的处理*/
    {
        ...
        ssd->channel_head[channel].chip_head[chip].next_state_predict_time =
                ssd->channel_head[channel].next_state_predict_time + ssd->parameter->time_characteristics.tBERS;
    ...
    } else if (command == NORMAL)                                             /*普通命令NORMAL的处理*/
    {
        ...
        ssd->direct_erase_count++;
        ssd->channel_head[channel].next_state_predict_time =
                ssd->current_time + 5 * ssd->parameter->time_characteristics.tWC;
        ssd->channel_head[channel].chip_head[chip].next_state_predict_time =
                ssd->channel_head[channel].next_state_predict_time + ssd->parameter->time_characteristics.tWB +
                ssd->parameter->time_characteristics.tBERS;
    } else {
        return ERROR;
    }
...
*********************************************************************************************************************/
int
gc_direct_erase(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane) {
	...
	*************************************************************************************************************************/
    if ((muilt_plane_flag == TRUE) && (interleaver_flag == TRUE) &&
    ...
    if ((normal_erase_flag ==
         TRUE))                              /*不是每个plane都有可以直接删除的block，只对当前plane进行普通的erase操作，或者只能执行普通命令*/
    {
...
Status move_page(struct ssd_info *ssd, struct local *location, unsigned int *transfer_size) {
    ...
    old_ppn = find_ppn(ssd, location->channel, location->chip, location->die, location->plane, location->block,
    ...
    if ((ssd->parameter->advanced_commands & AD_COPYBACK) == AD_COPYBACK) {
            ...
            1)                                                                /*贪婪地使用高级命令*/
        {
            ...
            while (old_ppn % 2 != ppn % 2) {
                ...
                ppn = get_ppn_for_gc(ssd, location->channel, location->chip, location->die,
                                     location->plane);    /*找出来的ppn一定是在发生gc操作的plane中，并且满足奇偶地址限制,才能使用copyback操作*/
                ssd->program_count--;
                ssd->write_flash_count--;
...
#ifdef DEBUGONLYGC
        printf("move_page has no copyback operation\n");
#endif
        (*transfer_size) += size(valid_state);
    }
    ssd->channel_head[new_location->channel].chip_head[new_location->chip].die_head[new_location->die].plane_head[new_location->plane].blk_head[new_location->block].page_head[new_location->page].free_state = free_state;
...
//lxcprogram
#ifdef DEBUGONLYGC
    printf("in GC, old location: channel %d, chip %d, die %d, plane %d, block %d, page %d                   change to new location: channel %d, chip %d, die %d, plane %d, block %d, page %d  \n", location->channel, location->chip, location->die, location->plane, location->block, location->page, new_location->channel, new_location->chip, new_location->die, new_location->plane, new_location->block, new_location->page );
#endif
    free(new_location);
    new_location = NULL;
...
********************************************************************************************************************************************/
//lxcprogram_gc there is no sub, so cant use the go_one_step. here is
//illustration.
//这些状态都是要进入的状态，而完成判断则是用下一个状态加上  时间判定，然后就走向下一个状态。

//for read send_read_cmd phase of once read_transfer_write process.
...
////for write_send_cmd part of once read_transfer_write process.
//ssd->channel_head[channel].current_state = CHANNEL_GC;
//    ssd->channel_head[channel].current_time = ssd->current_time;
//    ssd->channel_head[channel].next_state = CHANNEL_IDLE;
//    ssd->channel_head[channel].chip_head[chip].current_state = CHIP_GC_RW_BUSY;
...
//for verify of once write process write_data_from_register phase of once read_transfer_write process.
//    ssd->channel_head[channel].chip_head[chip].current_state = CHIP_GC_RW_BUSY;
//    ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
//    ssd->channel_head[channel].chip_head[chip].next_state = xxx ;//for write need 5 times!!!!! 

//    ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->current_time + ssd->parameter->time_characteristics.tPROG_w_verify;



//erase progress shown as follows   erase commands only have 5 instructions.
//对于开始，都是直接进入cmd发送状态，然后从cmd状态开始后走。所以刚开始没有idle状态。。都是cmd_transfer状态。

////for erase_send_cmd part of once read_transfer_write process.
//ssd->channel_head[channel].current_state = CHANNEL_GC;
//    ssd->channel_head[channel].current_time = ssd->current_time;
//    ssd->channel_head[channel].next_state = CHANNEL_IDLE;
//    ssd->channel_head[channel].chip_head[chip].current_state = CHIP_ERASE_BUSY;
...
//for verify phase of once erase process. 
//    ssd->channel_head[channel].chip_head[chip].current_state = CHIP_ERASE_BUSY;
//    ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
//    ssd->channel_head[channel].chip_head[chip].next_state = CHIP_IDLE; 
...
int
uninterrupt_gc(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane) {
...
//#define DEBUG_CURR_TIME
...
#ifdef DEBUG
    printf("begin go into uninterrupt_gc\n");
#endif
    ...
    //步骤： 1、find the victim block,but only once,bucause we will enter this function several times
    //       2、judge the read and write in gc_node. and then go into different phase.
    //       3、at last , jump into erase phase.


    // 1、find the victim block,but only once,bucause we will enter this function several times


    if(gc_node->victim_block_number == -1){
        if (find_active_block(ssd, channel, chip, die, plane) !=
                SUCCESS)                                           /*获取活跃块*/
        {
        ...
        block = -1;
        for (i = 0; i <
                ssd->parameter->block_plane; i++)                                                           /*查找最多invalid_page的块号，以及最大的invalid_page_num*/
        {
            total_invalid_page_num += ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].invalid_page_num;
            if ((active_block != i) &&
                    (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].invalid_page_num >
                     invalid_page)) {
...
#ifdef DEBUGONLYGC
                printf("everyGC only choose once block for victim.this block in  channel %d, chip %d, die %d, plane %d, block %d, has  invalid_page_num %d\n", channel, chip, die, plane, i, invalid_page);
#endif
        ...
        if (block == -1) {
            ...
            gc_node->victim_block_number = block;
            gc_node->block = block;
        }
    }
#ifdef DEBUGONLYGC
    ...
    //       2、judge the read and write in gc_node. and then go into different phase.
    if ((gc_node->read_write_end != 0) && ((current_state == CHIP_IDLE) || ((next_state == CHIP_IDLE) && (next_state_predict_time <= ssd->current_time)))){// this position only for channel IDLE and chip IDLE for the begining of (the read and write and erase phase in GC)

        //2.1 read begin in the read_transfer_write process.
        if( (gc_node->read == 1) && (gc_node->write == 0) && (gc_node->gc_read_inner == -1)) {

            gc_node->copy_page_number ++;
#ifdef DEBUG
            printf("here read will begin a new page, so page number ++ ,the new page is %d\n",gc_node->copy_page_number);
#endif
            for (i = gc_node->copy_page_number; i <
                    ssd->parameter->page_block; i++ )                                                             /*逐个检查每个page，如果有有效数据的page需要移动到其他地方存储*/
            {
                //here is the sign for last page. very important. since there are two situation for last page.

                if ((ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->victim_block_number].page_head[i].free_state &
                            PG_SUB) == 0xffffffff) {//change here.
                    gc_node->victim_block_freepage_statistic ++;
                }
                if (gc_node->victim_block_freepage_statistic != -1) {
                    printf("\ntoo much free page. \t %d\t .%d\t%d\t%d\t%d\t\n", gc_node->victim_block_freepage_statistic, channel, chip, die, plane);
                }
                if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->victim_block_number].page_head[i].valid_state >
                        0) /*该页是有效页，需要copyback操作*/
                {
                    ...
                    gc_node->copy_valid_number = i;
                    gc_node->copy_page_number = gc_node->copy_valid_number ;  
                    gc_node->gc_read_inner = 0;
#ifdef DEBUG
            printf("everytime the victim channel %d,chip %d, die %d, plane %d,  block %d page %d, , valid_state %d\n",channel, chip, die, plane, block, i, ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->victim_block_number].page_head[i].valid_state);
#endif

                    if(i == ((ssd->parameter->page_block) - 1)){
                        gc_node->last_page_in_victim_deal = 0;// this means the last page need (read and write).
                ...
                if(i == ((ssd->parameter->page_block) - 1)){
                    ...
                    // this means the last page don't have to deal, just go to erase directly.
                    gc_node->read_write_end = 0;
#ifdef DEBUG
                    printf("i is %d, means the last freepage,don't do copyback, now go to the begining of erase\n", i);
#endif
                    return 1;// return, next progress will enter erase, because there is no time changing here.

                }

            }

        }//end of the begining of the read.2.1


        //2.2 for write begin in the read_transfer_write process.
        if( (gc_node->read == 0) && (gc_node->write == 1) && (gc_node->gc_write_inner == -1)) {
#ifdef DEBUG
            printf("gc write phase, write iterations is %d,\n", gc_node->write);
#endif
            i = gc_node->copy_page_number;   /*here just once time, since the gc's write is after the read.*/


#ifdef DEBUG
            printf("GCwrite channel %d, chip %d, die %d, plane %d, block %d, page %d, valid_state is %d\n", channel, chip, die, plane, gc_node->victim_block_number, gc_node->copy_valid_number,ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->victim_block_number].page_head[gc_node->copy_valid_number].valid_state );
#endif


            if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->victim_block_number].page_head[gc_node->copy_valid_number].valid_state >
                    0) /*该页是有效页，需要copyback操作*/
            {
                ...
                // //lxcprogram_gc
#ifdef DEBUG
                printf("gc_node->copy_page_number is %d, i is %d,they are the same\n",gc_node->copy_page_number, i);
#endif
        ...
        }//end of the begining of write.

    }//end of the begining of read&write.




    //2.3 for erase pulse begin in the erase process.

    if ( (current_state == CHIP_IDLE) || ((next_state == CHIP_IDLE) && (next_state_predict_time <= ssd->current_time))){
        if (gc_node->read_write_end == 0) {// this position only for channel IDLE and chip IDLE for the begining of (the read and write and erase phase in GC)
            ...
            if(gc_node->erase == -1) {
                ...
                // //lxcprogram_gc
#ifdef DEBUG
                printf("gc_node->copy_page_number is %d, it is the end of read_transfer_write and the begin of the erase now\n",gc_node->copy_page_number);
#endif
                ////for erase_send_cmd part of once read_transfer_write process.
                ssd->channel_head[channel].current_state = CHANNEL_GC;
                ssd->channel_head[channel].current_time = ssd->current_time;
...
//    free_page = 0;
//    for (i = gc_node->copy_page_number; i <
//            ssd->parameter->page_block; )                                                             /*逐个检查每个page，如果有有效数据的page需要移动到其他地方存储*/
//        //lxcprogram_gc
//        unsigned int temp_number = 0;
//
//    {
//        if ((ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].free_state &
//                    PG_SUB) == 0xffffffff) {//change here.
//            gc_node->victim_block_freepage_statistic ++;
//        }
//        if (free_page != 0) {
//            printf("\ntoo much free page. \t %d\t .%d\t%d\t%d\t%d\t\n", gc_node->victim_block_freepage_statistic, channel, chip, die, plane);
//        }
//        if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].valid_state >
//                0) /*该页是有效页，需要copyback操作*/
//        {
//
//            //lxcprogram_gc
...
//            page_move_count++;
//            //lxcprogram_gc
//            gc_node->copy_valid_number += 1;
//            free(location);
//            location = NULL;
//        }
//        //lxcprogram_gc
//        //lxcprogram_gc
//        gc_node->copy_page_number += 1 ;  
//    }
//    erase_operation(ssd, channel, chip, die, plane,
...
//
//            ssd->channel_head[channel].next_state_predict_time = ssd->current_time + page_move_count * (7 *
//                    ssd->parameter->time_characteristics.tWC +
//                    ssd->parameter->time_characteristics.tR +
//                    7 *
//                    ssd->parameter->time_characteristics.tWC +
//                    ssd->parameter->time_characteristics.tPROG);
//            ssd->channel_head[channel].chip_head[chip].next_state_predict_time =
//                ssd->channel_head[channel].next_state_predict_time + ssd->parameter->time_characteristics.tBERS;
//        }
//    } else {
...
********************************************************************************************************************************************/
int interrupt_gc(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane,
                 struct gc_operation *gc_node) {
    ...
    if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->block].invalid_page_num !=
        ssd->parameter->page_block)     /*还需要执行copyback操作*/
    {
        for (i = gc_node->page; i < ssd->parameter->page_block; i++) {
            if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->block].page_head[i].valid_state >
                0) {
                location = (struct local *) malloc(sizeof(struct local));
                alloc_assert(location, "location");
                memset(location, 0, sizeof(struct local));
                ...
                if ((ssd->parameter->advanced_commands & AD_COPYBACK) == AD_COPYBACK) {
                    ssd->channel_head[channel].next_state_predict_time =
                            ...
                            ssd->parameter->time_characteristics.tR + 7 * ssd->parameter->time_characteristics.tWC;
                    ssd->channel_head[channel].chip_head[chip].next_state_predict_time =
                            ssd->channel_head[channel].next_state_predict_time +
                            ssd->parameter->time_characteristics.tPROG;
                } else {
                    ssd->channel_head[channel].next_state_predict_time = ssd->current_time +
                                                                         (7 + transfer_size * SECTOR) *
                                                                         ssd->parameter->time_characteristics.tWC +
                                                                         ssd->parameter->time_characteristics.tR +
                                                                         (7 + transfer_size * SECTOR) *
                                                                         ssd->parameter->time_characteristics.tWC;
...
#ifdef DEBUGONLYGC
        printf("channel gc only one,so here\n");
#endif
...
#ifdef DEBUG
                printf("channel %d, delete gc_node address %d\n", channel, gc_node);
#endif
...
//        next_state = ssd->channel_head[channel].chip_head[gc_node->chip].next_state;
//        next_state_predict_time = ssd->channel_head[channel].chip_head[gc_node->chip].next_state_predict_time;
//        if ((current_state == CHIP_IDLE) ||
//            ((next_state == CHIP_IDLE) && (next_state_predict_time <= ssd->current_time))) {
...
//    }
    //lxcprogram_gc   这个地方很重要，因为自己要一直进入，所以只能将每次chip判断放在后面了。
    if (gc_node->priority == GC_UNINTERRUPT)                                     /*这个gc请求是不可中断的，优先服务这个gc操作*/
            {
                flag_priority = 1;
            }
#ifdef DEBUG
    printf("gc_node is %x, and flag_priority(must be 1) is %d.gc_node mustn't be NULL\n", *gc_node, flag_priority);
#endif

    if (flag_priority != 1)                                                              /*没有找到不可中断的gc请求，首先执行队首的gc请求*/
    {
#ifdef DEBUG
        printf("errors happend in gc1   !!!\n");
#endif
...
if (gc_node->priority == GC_UNINTERRUPT) {
		//lxcprogram_gc
#ifdef DEBUG 
printf("every time gc only happen here\n");

#endif
uninterrupt_gc(ssd, channel, chip, die, plane);
...
//            flag_gc = uninterrupt_gc(ssd, channel, chip, die,
//                                     plane);                         /*当一个完整的gc操作完成时（已经擦除一个块，回收了一定数量的flash空间），返回1，将channel上相应的gc操作请求节点删除*/
//            if (flag_gc == 1) {
...
//    }
	...
	********************************************************************************/
        ...
        flag_invoke_gc = decide_gc_invoke(ssd,
                                          channel);                                  /*判断是否有子请求需要channel，如果有子请求需要这个channel，那么这个gc操作就被中断了*/

        if (flag_invoke_gc == 1) {
            flag_direct_erase = gc_direct_erase(ssd, channel, chip, die, plane);
            if (flag_direct_erase == -1) {
                flag_gc = interrupt_gc(ssd, channel, chip, die, plane,
                                       gc_node);             /*当一个完整的gc操作完成时（已经擦除一个块，回收了一定数量的flash空间），返回1，将channel上相应的gc操作请求节点删除*/
                if (flag_gc == 1) {
                    delete_gc_node(ssd, channel, gc_node);
                }
...
unsigned int gc(struct ssd_info *ssd, unsigned int channel, unsigned int flag) {
    ...
    if (flag == 1)                                                                       /*整个ssd都是IDEL的情况*/
    {
        for (i = 0; i < ssd->parameter->channel_number; i++) {
            ...
            gc_node = NULL;
            gc_p = NULL;
            if ((ssd->channel_head[i].current_state == CHANNEL_IDLE) ||
                (ssd->channel_head[i].next_state == CHANNEL_IDLE &&
                 ssd->channel_head[i].next_state_predict_time <= ssd->current_time)) {
                channel = i;
                if (ssd->channel_head[channel].gc_command != NULL) {
...
//lxcprogram_gc
        ...
        if ((ssd->parameter->allocation_scheme == 1) ||
            ((ssd->parameter->allocation_scheme == 0) && (ssd->parameter->dynamic_allocation == 1))) {
          //  if ( (ssd->channel_head[channel].subs_r_head != NULL) )    /*队列上有请求，先服务请求*/
          //  {
          //      return 0;
          //  }
        }
...
int decide_gc_invoke(struct ssd_info *ssd, unsigned int channel) {
    ...
    if ((ssd->channel_head[channel].subs_r_head == NULL) &&
        (ssd->channel_head[channel].subs_w_head == NULL))    /*这里查找读写子请求是否需要占用这个channel，不用的话才能执行GC操作*/
    {
        ...
        if (ssd->channel_head[channel].subs_w_head != NULL) {
            ...
            while (sub != NULL) {
                if (sub->current_state ==
                    SR_WAIT)                                         /*这个读请求是处于等待状态，如果他的目标die处于idle，则不能执行gc操作，返回0*/
                {
                    location = find_location(ssd, sub->ppn);
                    if ((ssd->channel_head[location->channel].chip_head[location->chip].current_state == CHIP_IDLE) ||
                    ...
                    location = find_location(ssd, sub->ppn);
                    if (ssd->channel_head[location->channel].chip_head[location->chip].next_state_predict_time <=
                        ssd->current_time) {
                        free(location);
                        location = NULL;
                        return 0;
