    ...
    if(temp_num_request == 0){
        ...
        while(temp_request != NULL){
            ...
            while(sub != NULL){
                ...
                {
                    printf("sub status channle: %d, chip: %d, die %d, plane %d, lpn: %d, ppn: %d  operation: %d,size: %d,   ////sub_request status:: current_state: %d, current_time: %llu, next_state: %d, next_state_predict_time: %llu, state(only for write): %d\n",sub->location->channel, sub->location->chip, sub->location->die, sub->location->plane,  sub->lpn, sub->ppn,sub->operation, sub->size, sub->current_state, sub->current_time,sub->next_state, sub->next_state_predict_time, sub->state) ;
                }
...
//printf("ssd first request time is %lld\n",  );
            ...
            //since always be stalled here.then printf gc
            unsigned int temp_channel_number = 0;

            struct gc_operation *gc_node_temp = NULL;
            //printf("gc number is %d\n", ssd->gc_request);
//            for(temp_channel_number = 0;  temp_channel_number < ssd->parameter->channel_number; temp_channel_number++ )
//            {
//                printf("gc print loop is here\n"); 
//                gc_node_temp = ssd->channel_head[temp_channel_number].gc_command;
//                if(gc_node_temp != NULL){
//                    printf(" befor service gc, gc_node information is gc_node, chip %d, die %d, plane %d,  gc_node->victim_block_number is %d, gc copy_page_number is %d, copy_valid_number is %d, pos in copy is %d,  read %d, write %d, erase %d, read_inner %d, write inner %d, read_write_end %d, last_page_in_victim %d, victim_block_freepage %d, chip_status %d, sign for_preemptive %d, chip_next_predict_time %lld\n", gc_node_temp->chip, gc_node_temp->die, gc_node_temp->plane, gc_node_temp->victim_block_number,gc_node_temp->copy_page_number, gc_node_temp->copy_valid_number, gc_node_temp->pos_in_copy, gc_node_temp->read, gc_node_temp->write, gc_node_temp->erase, gc_node_temp->gc_read_inner, gc_node_temp->gc_write_inner, gc_node_temp->read_write_end, gc_node_temp->last_page_in_victim_deal, gc_node_temp->victim_block_freepage_statistic, gc_node_temp->chip_status, gc_node_temp->sign_for_preemptive, gc_node_temp->chip_next_predict_time_diff);
//                }       
//            }
