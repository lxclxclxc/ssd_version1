...
//                    printf("fixed capacity is wrong. lxclxclxc_lpnwrong in now\n");
                }
               ...
               NULL)                                                               /*if there are read requests in queue, send one of them to target chip*/
        {
            ...
            if (sub->current_state == SR_WAIT) {
                ...
                if ((ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].current_state ==
                    ...
                    //lxcprogram_for_adjust_write_suspension_number
//                    //only stopped, when there is no gc.
//
//                    //if((ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_read_stop == 1) && (ssd->channel_head[sub->location->channel].gc_command == NULL) ){
//                   unsigned int temp_num_write = 0; 
//                    temp_num_write = judge_write_existing_in_chip(ssd, sub->location->channel, sub->location->chip);
//                    if(temp_num_write == 1){
//                        if(ssd->channel_head[sub->location->channel].gc_command == NULL ){
//                            if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_read_real_stop == 1) {
//
//                                printf("for adjust write suspension number\n");
//                                break;
//
//                            }
//                        }
