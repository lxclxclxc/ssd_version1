...
find_read_sub_request(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die) {
    ...
    for (plane = 0; plane < ssd->parameter->plane_die; plane++) {
        ...
        printf("we want to find existing read sub from channel %d, chip %d, die %d, now the existing register_ppn is %d, if ppn is -1, that means no useful\n", channel, chip, die, address_ppn);


        if (address_ppn != -1) {
            if(ssd->channel_head[channel].subs_r_head == NULL){
                printf("if this channel %d, has address_ppn, that means at least one subrequest here..,since if one readsub finish, add_reg_ppn will be -1\n");
                while(1){}
            }
            ...
            while (sub != NULL) {
                printf("we list all readsub in channel %d, sub is %lld\n", channel, sub);
                if (sub->ppn == address_ppn) {
                   // p = sub->next_node;
                    //lxcprogram_gc original wrong.
                    return sub;
...
//                    sub = p;
 ...
 if(sub == NULL){    
        printf("wrong!!, since here address_ppn is useful\n");
//        while(1){}
 }
    ...
    if(sub != NULL){    
        printf("wrong!!, since I just return if we can find it\n");
        while(1){}
            if (sub->ppn == address_ppn) {
                sub->next_node = NULL;
...

{
    ...
    int temp_for_printf_chip_subrequest_length = 0;
#ifdef DEBUGSUSPEND
            ...
            ssd->parameter->channel_number; i++)                                       /*for the preemptive judgment and service of write operation*/
    {
        ...
        for(j = 0; j < ssd->channel_head[i].chip; j++){ 
...
#ifdef  DEBUGADJUSTWRITE 
                    printf("here channel %d, chip %d is not write  status ! jump out.\n", i, j);
#endif
            ...
            //3) every time,after the finishment of one process,then try to suspend the write.here use write_try_to_suspend_function.  note this:  only the condition ( current_time > chip_next_state_predict_time) is true, then begin the next process.
                ...
                if(ssd->channel_head[i].chip_head[j].current_state == CHIP_WRITE_DATA_CMD_BUSY){         
                    ...
                    if ( (sub->current_state == SR_W_C_A_DATA_TRANSFER) && (sub->next_state == SR_W_DATA_TRANSFER) ) {
                        ...
                        if(sub_for_read != NULL){ 
                            ...
                            printf("write suspend position is in just finish send_read_cmd . num_w_cylce is  %d, and the readsub is %lld\n",ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle, sub_for_read );
#endif
                    ...
                    //lxcprogram_for_adjust_write_suspension_number
                        //the most important thing is the steps number constant (NUMBER_OF_WRITE_ITERATION) cannot be less than 2, at least 2. 
...
printf("statistic_suspendsion_write, 0\n");
            ...
            printf("storesub  operation. chip: %d, lpn: %d, ppn: %d  operation: %d,size: %d,   ////sub_request status:: current_state: %d, current_time: %llu, next_state: %d, next_state_predict_time: %llu, state(only for write): %d\n", sub->location->chip, sub->lpn, sub->ppn,sub->operation, sub->size, sub->current_state, sub->current_time,sub->next_state, sub->next_state_predict_time, sub->state) ;
                ...
                if( (ssd->channel_head[i].chip_head[j].num_w_cycle <= 10) && (ssd->channel_head[i].chip_head[j].num_w_cycle >= 1) ){ 
                    ...
                    if ((sub->next_state_predict_time <= ssd->current_time) && (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle == 10) ) {
                        ...
                        temp_for_printf_chip_subrequest_length = 1;
...
//printf here
//                        printf_channel_chip_timestamp(ssd);


//lxcprogram_for_adjust_write_suspension_number
//judge the wrong situation here.
                        if((ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write == 1) && (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write == 1)){
                            printf("adjusting signs are wrong now\n");
                            while(1){}
                        }
//the most most most important thing is that we should put channel_for_write_sub and chip_for_write_sub only at new write sub will happen, not the restore old writesub as above
//only here and when gc happen to clear the value of sign for writesub
                        ssd->channel_head[sub->location->channel].channel_for_write_sub = 0;
                        ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_for_write_sub = 0;


                        ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin = 0;


                        if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write == 1){

                            ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write = 0;
                        }



                        if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write == 1){

                            ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write = 0;
                        }

                        if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_change_write_iterations_number == 1){

                            ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_change_write_iterations_number = 0;

                        }
                        //lxcprogram_for_adjust_write_suspension_number

                        printf("nowwritesub %lld, in channel %d, chip %d, has been finished\n", sub, sub->location->channel, sub->location->chip);
...
//                        printf("adjust_write, channel %d, chip %d, chip_write_record_begin is %d, NUMBER + chip_begin is %d, moving num_w_cycle is %d\n",sub->location->channel, sub->location->chip, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin,NUMBER_OF_WRITE_ITERATION + ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin,ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle) ;
...
//2) adjusting for steps less than 10

//2.1) reach the end of big steps
...
printf("judgement in adjustingwriteiteraion \n");
//must add here!!!
    /*******************judge for suspend **************************/
                                    //judge for whether need suspend. if so, here store the next num_w_cycle value here is 1.
                                    //but the most important thing here is !!!!!! whether the read and serviced write are  in the same chip. 
                            ...
                            printf("write suspend position is in just finish send_read_cmd . num_w_cylce is  %d, and the readsub is %lld, chip_haveto_find_a_new_write is %d, only_needto_restore_suspend_write is %d\n",ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle, sub_for_read, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write );
...
//    /*******************judge for suspend **************************/
//if(Has_read_request == 1){
//printf("now there is read\n");
//}

//2.1)here should continue go for write
                                    }else{
                                        //just ignore read
                        ...
                        }  //this brace is for 2)adjusting for steps less than 10

}//here if is just for adjust situation, and else is for normal situation

else{


                                printf("judgement in normalwriteiteraion \n");
                                /*******************judge for suspend **************************/
                                //judge for whether need suspend. if so, here store the next num_w_cycle value here is 1.
                                //but the most important thing here is !!!!!! whether the read and serviced write are  in the same chip. 
#ifdef DEBUGSUSPEND
                    ...
                    }//end of normal write suspension and for 1)adjusting steps
...
if( (Has_read_request == 1) && (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_change_write_iterations_number == 1) ){

printf("adjust_write error !!!\n");
while(1){}
}
...
//so important, that make me confused!!!so much.
                                    ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin = 0;
...
//so important, that make me confused!!!so much.
                        ...
                        printf("writesuspension information sub %lld, channel %d, chip %d, chip_write_record_begin is %d, NUMBER + chip_begin is %d, the result of num_w_cycle is %d\n",sub, sub->location->channel, sub->location->chip, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin,NUMBER_OF_WRITE_ITERATION + ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin,ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle) ;
                            ssd->write_suspend_times ++;
                            printf("write suspend times is %d\n", ssd->write_suspend_times);
                            ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].write_preemptive_sign = 1;
//                            ssd->current_time += (7+2) * ssd->parameter->time_characteristics.tWC;
...
//*************only for print chip request length*******************//
if(temp_for_printf_chip_subrequest_length == 1){
            printf_every_chip_static_subrequest(ssd);

            temp_for_printf_chip_subrequest_length = 0;
}
...
//*************only for print chip request length*******************//
            ...
            ssd->parameter->channel_number; i++)                                       /*这个循环处理不需要channel的时间(读命令已经到达chip，chip由ready变为busy)，当读请求完成时，将其从channel的队列中取出*/
    {
        ...
        while (sub != NULL) {
#ifdef DEBUGADJUSTWRITE             
            printf("here is the whole read in channel %d, sub is %lld\n", i, sub); 
#endif
            //printf("lxc,now is wrong read here.channel is %d, \n", i);
            if(sub->location == NULL){
                printf("lxc,now is wrong read here.channel is %d, \n", i);
     ...
     //       printf("wrong sub is %lld \n", sub);
         ...
         if ((sub->current_state == SR_COMPLETE) ||
                    ...
                    printf("here is can be moved now channel %d, chip is %d,  head address %lld, tail address %lld, moved sub is %lld\n", i, sub->location->chip, ssd->channel_head[i].subs_r_head,ssd->channel_head[i].subs_r_tail, sub);


            //.....................lxcprogram new frame for read and write scheduler.......................//
//finished subrequest must be set to NULL waiting for another feed at services_2_r_wait. But now we found that there is no need here.!!!!!!

             
//lxcprogram_for_adjust_write_suspension_number and 
//!!!!!!!!!!here we need to not only adjust the write number, but also should give the same chip's write subrequest service right.!!!! 
//for delete completed subrequests is tracing from channel queue. here wo should take into consider more chips

//after deleted, judge who can take the priority, read or write here.
     unsigned int temp_num_write_request = 0; 
     unsigned int temp_exist_read_subrequest = 0; 
     unsigned int temp_exist_write_subrequest = 0; 
     unsigned int temp_length;
     unsigned int temp_chip;
     temp_length =  WRITE_RATIO_IN_REQUEST_QUEUE * ssd->parameter->queue_length;
     temp_num_write_request = statistic_write_request_number(ssd);
//why use this? since the chip is basic element 
//     for (i = 0; i < ssd->parameter->channel_number; i++) { 

//         gc_node = ssd->channel_head[i].gc_command;
//when is there no GC now. Here GC is not the newest,needing update later at read first step. 
//         if(gc_node != NULL){
//             temp_chip = gc_node->chip ;
//         }

//         for (j = 0; j < ssd->parameter->chip_channel[i]; j++) {
//from here, we just find when read should give priority,  shown as followd
//when the write requests number excceed the threshold
     if( (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write == 0) && (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write == 0) ){
         printf("has no any write priority sign now, should give writesub priority now\n");

//here mask the temp_length for fixed iterations

       //  temp_length = ssd->parameter->queue_length;
//here mask the temp_length for fixed iterations

         


         if( temp_num_write_request >= temp_length )  {
             printf("traffic jam, but not sure to go\n");
             temp_exist_write_subrequest = judge_write_existing_in_chip(ssd,sub->location->channel,sub->location->chip);
             temp_exist_read_subrequest = judge_2read_existing_in_chip(ssd, sub->location->channel,sub->location->chip);
if(temp_exist_write_subrequest == 0){

#ifdef DEBUGADJUSTWRITE 
             printf("traffic jam, channel %d, chip %d, has no write sub now\n", sub->location->channel, sub->location->chip);
#endif
         }


             if( ( temp_exist_write_subrequest >= 1 ) && (temp_exist_read_subrequest >= 2) ){


                 printf("traffic jam now!, information is channel %d, chip %d, writerequest length threshold is %d, num_write_request is %d,  exist_write_subrequest is %d, exist_read_subrequest is %d\n", sub->location->channel,sub->location->chip, temp_length, temp_num_write_request, temp_exist_write_subrequest, temp_exist_read_subrequest);

                 if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle > 0){

                     printf("traffic middle restore now\n");

                     printf("channel %d, chip %d, has sub %lld, num_w_cycle %d>0\n", sub->location->channel, sub->location->chip, sub, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle);

                     ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_only_needto_restore_suspended_write = 1;


                     try_to_restore_write_function_during_adjust_write_iterations(ssd, sub->location->channel,sub->location->chip);



//so important!!!!!!now!!!!for unlock the write subrequest.
ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].write_preemptive_sign = 0;


                 }                //if (gc_node == NULL)  {//here should not have to judge gc_node, we can judge gc at read begin, here is just for setting iteration numbers and if has gc, we can jump 
                 else{

                     printf("traffic find a new write one\n");

                     ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_haveto_find_a_new_write = 1;
                     //the most most most important thing is that we should put channel_for_write_sub and chip_for_write_sub only at new write sub will happen, not the restore old writesub as above

                     ssd->channel_head[sub->location->channel].channel_for_write_sub = 1;
                     ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_for_write_sub = 1;
                 }

                 ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_change_write_iterations_number = 1;

                 ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_record_begin = ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle;


                 printf("channel %d, chip %d, execute the adjust_write now , begin_write_record is %d\n", sub->location->channel, sub->location->chip, ssd->channel_head[sub->location->chip].chip_head[sub->location->channel].num_w_cycle);



             }//end of existing write judging.

             //                         }//end of no GC 

         }//end of write requests length judgement
         //                 }//end of chip_real_read
     else{



         printf("normal one step alternately go ahead with read and write, but not certainly go ahead\n");

         temp_exist_write_subrequest = judge_write_existing_in_chip(ssd,sub->location->channel,sub->location->chip);
         temp_exist_read_subrequest = judge_2read_existing_in_chip(ssd, sub->location->channel,sub->location->chip);
         //   if( (gc_node == NULL) || ((gc_node != NULL) && (temp_chip != j))  ) {
         if(temp_exist_write_subrequest == 0){

#ifdef DEBUGADJUSTWRITE 
             printf("normal one step channel %d, chip %d, has no write sub now\n", sub->location->channel, sub->location->chip);
#endif
         }
         ...
         if( ( temp_exist_write_subrequest >= 1 ) && (temp_exist_read_subrequest >= 2) ){
             ...
             printf("normal one step alternately go ahead with read and writei,before go ahead, information is channel %d, chip %d, writerequest length threshold is %d, num_write_request is %d,  exist_write_subrequest is %d, exist_read_subrequest is %d\n", sub->location->channel,sub->location->chip, temp_length, temp_num_write_request, temp_exist_write_subrequest, temp_exist_read_subrequest);
             if(ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle > 0){


                 printf("middle restore now\n");
                 printf("channel %d, chip %d, has sub %lld, num_w_cycle %d>0\n", sub->location->channel, sub->location->chip, sub, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].num_w_cycle);
...
ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].write_preemptive_sign = 0;
     ...
     //             }//end of chip loop
     //         }//end of channel loop
         }//end of if already setting, dont set again.
         else{
             printf("now channel%d, chip %d, already has write priority sign now, dont give again\n", sub->location->channel, sub->location->chip);
...
//only for judge test
                if( ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].assigned_willbe_dealed_read_sub != sub){// here is the only way to assign read subrequst for this chip
                        
                        printf("read subrequest has wrong in channel %d, chip %d\n",sub->location->channel, sub->location->chip);
                        while(1){}
                        }
                    ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].assigned_willbe_dealed_read_sub = NULL;
                ...
                if (sub !=
                        ssd->channel_head[i].subs_r_head)                             /*if the request is completed, we delete it from read queue */
                {
                    printf("now deleted subqueue is not channel %dhead\n",i);
                  //  p->next_node = sub->next_node;
                   ...
                   //     p = sub; // now p should stay here!!!
                    ...
                    if(ssd->channel_head[i].subs_r_head->location == NULL){   
                   ...
                   // ssd->channel_head[i].chip_head[chip].chip_read_queue_length --;
                    ...
                    printf("after moved now channel %d, length is %d,  head address %lld, tail address %lld, \n", i, tempchannel_read_length, ssd->channel_head[i].subs_r_head,ssd->channel_head[i].subs_r_head,ssd->channel_head[i].subs_r_tail,ssd->channel_head[i].subs_r_tail );
                   if((tempchannel_read_length != 0) && ( ssd->channel_head[i].subs_r_head == NULL)){
                   printf("should check here\n");
                   while(1){}
                   }
                    //printf_every_chip_read_subrequest(ssd);
            }else {// end of complete or c_a_cmd  state
                //here means if sub is not in cmd state, then judge next.
            p = sub;
            sub = sub->next_node;
...
//             while(1){}
//         }
        }// end of complement judgement.while loop
...
//             //from here, we just find when read should give priority,  shown as followd
//             //when the write requests number excceed the threshold
//             if(ssd->channel_head[i].chip_head[j].chip_read_real_stop == 0){
//                 if( temp_num_write_request >= temp_length )  {
//                     temp_exist_write_subrequest = judge_write_existing_in_chip(ssd, i, j);
//                     temp_exist_read_subrequest = judge_2read_existing_in_chip(ssd, i, j);
//                     printf("before adjust write iterations here channel %d, chip %d, temp_length is %d, temp_exist_write is %d, temp_num_write is %d, temp_exist_read is %d\n", i, j, temp_length, temp_exist_write_subrequest, temp_num_write_request, temp_exist_read_subrequest);
//                  //   if( (gc_node == NULL) || ((gc_node != NULL) && (temp_chip != j))  ) {
//
//if(ssd->channel_head[i].chip_head[j].chip_write_record_begin->num_w_cycle > 0){
//                                 try_to_restore_write_function_during_adjust_write_iterations(ssd, i, j);
//}                //if (gc_node == NULL)  {//here should not have to judge gc_node, we can judge gc at read begin, here is just for setting iteration numbers and if has gc, we can jump 
//
//                         if( ( temp_exist_write_subrequest >= 1 ) && (temp_exist_read_subrequest >= 1) ){
//
//                            // if(ssd->channel_head[i].chip_head[j].num_w_cycle == 0){
//                                 ssd->channel_head[i].chip_head[j].chip_read_real_stop = 1;
//                                 //ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].storedsub = ;
//
//                                 ssd->channel_head[i].chip_head[j].chip_write_record_begin = ssd->channel_head[i].chip_head[j].num_w_cycle;
//
//
//                                 printf("channel %d, chip %d, execute the adjust_write now , begin_write_record is %d\n", i, j, ssd->channel_head[i].chip_head[j].num_w_cycle);
//
...
//if(){
//
//setting_write_subrequest_in_corresponding_chip();//this function is deciding which write subrequest is the first one.
//}
...
//.....................lxcprogram new frame for read and write scheduler.......................//
int judge_any_wait_state_read_subrequest_here(struct ssd_info* ssd, int channel, int chip)
{
    int read_in_waitstate_number = 0;
    struct sub_request *temp_sub = NULL;
    temp_sub = ssd->channel_head[channel].subs_r_head;
    while(temp_sub != NULL){

        if(temp_sub->location->chip == chip){
            if(temp_sub->operation == 1){//no useful here.
                if(temp_sub->current_state == SR_WAIT){
                    read_in_waitstate_number ++;
                }
            }
    ...
    return read_in_waitstate_number;
...
//int judge_any_complete_read_subrequest_here(struct ssd_info* ssd, int channel, int chip)
//{
//
//
//
...
//.....................lxcprogram new frame for read and write scheduler.......................//
// here should pay attention that the result sub_request should stay at SR_WAIT state.
struct sub_request*  after_scheduler_to_get_readsubrequest(struct ssd_info* ssd, int channel, int chip)
{

    struct sub_request *temp_read_subrequest = NULL;



// 1 strategy, normally choose the first subrequest queued in this chip.
#ifdef first_go_strategy
    temp_read_subrequest = ssd->channel_head[channel].subs_r_head;

    while(temp_read_subrequest != NULL){
        if(temp_read_subrequest->location->chip == chip){

            if (temp_read_subrequest->current_state == SR_WAIT){

            break;
            }
        }
        temp_read_subrequest = temp_read_subrequest->next_node; 
...
// 2 stratedy, from the slack-time cost,  choose the read subrequest. 
#ifdef slack_time_strategy
...
//there is NULL queue number judgment, so if the result is NULL, will report error here.

    if( temp_read_subrequest == NULL){

        printf("wrong in after_scheduler_to_get_readsubrequest\n");
        while(1){}

    }   
    return temp_read_subrequest; 
}
...
struct sub_request*  after_scheduler_to_get_writesubrequest(struct ssd_info* ssd, int channel, int chip)
{
struct sub_request *temp_write_subrequest = NULL;



// 1 strategy, normally choose the first subrequest queued in this chip.
#ifdef first_go_strategy
    temp_write_subrequest = ssd->channel_head[channel].subs_w_head;

    while(temp_write_subrequest != NULL){
        if(temp_write_subrequest->location->chip == chip){

            if (temp_write_subrequest->current_state == SR_WAIT){
                break;
            }
        }
        temp_write_subrequest = temp_write_subrequest->next_node; 
...
//there is NULL queue number judgment, so if the result is NULL, will report error here.
    ...
    if( temp_write_subrequest == NULL){

        printf("wrong in after_scheduler_to_get_writesubrequest\n");
        while(1){}

    }   
    return temp_write_subrequest; 
...
//
	...
	*ssd不能执行执行高级命令的情况下
    ...
    if (((ssd->parameter->advanced_commands & AD_INTERLEAVE) != AD_INTERLEAVE) &&
        ...
        //#endif
        //  //controll read write change, and the change time. and write execute also need find the first one. this means one write will block the channel a lot time 
        //            find_skip_read_for_adjust_write();
        //if(){
        //
        //
        //}
        ...
        unsigned int temp_readsubrequest_here; 
        unsigned int temp_chip_number = 0;
        unsigned int temp1 = 0;
        unsigned int find_subread = 0;
        //        unsigned int temp2 = 0;
        //        unsigned int temp1 = 6;

        unsigned int chip_begin_token = ssd->channel_head[channel].chip_readtoken + 1;
        unsigned int chip_number_setting = ssd->channel_head[channel].chip_readtoken + ssd->parameter->chip_num / ssd->parameter->channel_number;
        struct sub_request* assigned_read_request = NULL;
...
#ifdef DEBUGADJUSTWRITE             
printf("we want every sub can be polled if no sub can be serviced, now sub is %lld\n", sub);
#endif
        for(temp1 =chip_begin_token; temp1 <=chip_number_setting; temp1 ++){
//            printf("is there loop here 2??\n");
            //do model value here

            if(temp1 > ((ssd->parameter->chip_num / ssd->parameter->channel_number)-1)){
                temp_chip_number = temp1 - (ssd->parameter->chip_num / ssd->parameter->channel_number);

            }else{


                temp_chip_number = temp1;
            }


            //                while (sub != NULL){
            //
            //                temp_chip_number = sub->location->chip;

            //            printf("channel %d, temp_chip_number is %d\n", channel, temp_chip_number);

            if ((ssd->channel_head[channel].chip_head[temp_chip_number].current_state ==
                        CHIP_IDLE) ||
                    ((ssd->channel_head[channel].chip_head[temp_chip_number].next_state ==
                      CHIP_IDLE) &&
                     (ssd->channel_head[channel].chip_head[temp_chip_number].next_state_predict_time <=
                      ssd->current_time))) {


#ifdef DEBUGADJUSTWRITE             
                printf("channel %d, chip %d, now is IDLE, sub judgement is %lld\n", channel, temp_chip_number, sub);
#endif
                if( (sub->current_state != SR_WAIT) || (sub->location->channel != channel) || (sub->location->chip != temp_chip_number) ){

#ifdef DEBUGADJUSTWRITE             
                    printf("sub %lld, sub's channel %d, chip %d, is not serviced in channel %d, chip %d, so change chip or change sub??, we choose cherish the \n", sub, sub->location->channel, sub->location->chip,  channel, temp_chip_number);
#endif
                    continue;
                }
                ...
                //!!!!!!!the most important changes here are I changed the runing element from channel's sub to every chip itself, liking write subrequest, but the problem is that always begin in the first chip, we should make a token to record the write chipnumber and read chipnumber at any time.


                //step3:there is chipforreadat least read subrequest here, 
                //step4:
                //step5:
                //step6:
                ...
                //here is assignment for new read subrequest in this appointed chip. because here is already chipIDLE, so here must feed the chip new read here. 
                //certainlly, next step should be judging whether read need to give up for if the write subrequest has the higher priority in this chip now. 

                //lxcprogram new frame for read and write scheduler.

                //step1: if there is no complete read subrequest, then just return directly.here at least one of two.   there is one of two conditions to be satisfied here,then that means read subrequst is here .
                //only test here
                if( (ssd->channel_head[channel].chip_head[temp_chip_number].chip_haveto_find_a_new_write == 1) && (ssd->channel_head[channel].chip_head[temp_chip_number].chip_only_needto_restore_suspended_write == 1) ){
                ...
                temp_readsubrequest_here = judge_any_wait_state_read_subrequest_here(ssd, channel, temp_chip_number);
                if(temp_readsubrequest_here < 1){

            printf("channel %d, chip %d, no await read subrequest,return and next chip \n",channel, temp_chip_number);
                    continue;
                }else{


                    //only test here
                    if(ssd->channel_head[channel].chip_head[temp_chip_number].assigned_willbe_dealed_read_sub != NULL){
                        printf("wrong in the begin of read, since channel and chip are all IDLE, means there is no subrequest serviced here!!!\n");
                        while(1){}

                    }

                    //actually we have assigned who(read or write) can get the  write subrequest in this chip, buif write get the priority in this chip, then read should give up, except for if there is gc will happening here. we have to stop the write, for the GC which will happen. !!!!!the most important thing is that the earliest writesubrequest assignment for chip is not the newest GC happening time,(the finish time of read subrequest in this chip), so here, we need another gc judgement here!!!!

                    if( (ssd->channel_head[channel].chip_head[temp_chip_number].chip_haveto_find_a_new_write == 0) ||( (ssd->channel_head[channel].chip_head[temp_chip_number].chip_haveto_find_a_new_write == 1) && (ssd->channel_head[channel].gc_command != NULL ) ) ){
                        //step2: after skipping from two situation, we now can get new read subrequest here, undering two situation.!!!!!the most important is that we choose a subrequest is staying at wait state. 
                 //       assigned_read_request = after_scheduler_to_get_readsubrequest(ssd, channel, temp_chip_number);
                        assigned_read_request = sub;
                        if(assigned_read_request == NULL){
                            printf("wrong in after_scheduler_to_get_readsubrequest 1\n");
                            while(1){}

                        }

                        if (assigned_read_request->current_state != SR_WAIT) {
                            printf("wrong in after_scheduler_to_get_readsubrequest 2\n");
                            while(1){}
                        }

                        ssd->channel_head[channel].chip_head[temp_chip_number].assigned_willbe_dealed_read_sub = assigned_read_request;// here is the only way to assign read subrequst for this chip

            printf("now, here will do execute new read subrequest, then over the chip loop here.is %lld, channel %d, chip %d , chip token is %d\n", assigned_read_request, channel, temp_chip_number, temp_chip_number);
                    }//end of there is existing readrequest here.
                    //here is the real beginning of read service then break direct.

                    if(ssd->channel_head[channel].chip_head[temp_chip_number].assigned_willbe_dealed_read_sub != NULL){

                        if( (sub->location->channel != channel) || (sub->location->chip != temp_chip_number) ){
printf("channel, chip wrong\n");
while(1){}
                        }
                        go_one_step(ssd, assigned_read_request, NULL, SR_R_C_A_TRANSFER, NORMAL);

                        *change_current_time_flag = 0;
                        *channel_busy_flag = 1;                                              /*已经占用了这个周期的总线，不用执行die中数据的回传*/
                        ssd->channel_head[channel].chip_readtoken = temp_chip_number;
                        find_subread = 1;
                        //the most most most important thing is that we should put channel_for_write_sub and chip_for_write_sub only at new write sub will happen, not the restore old writesub as above

                        //here incase for gc happen here.
                        if( ssd->channel_head[channel].channel_for_write_sub == 1 ){

                            ssd->channel_head[channel].channel_for_write_sub = 0;

                        }

                        if(ssd->channel_head[channel].chip_head[temp_chip_number].chip_for_write_sub == 1 ){
                            ssd->channel_head[channel].chip_head[temp_chip_number].chip_for_write_sub = 0;
                        }
                        break;// one channel only service one chip at once time, so have to jump out here.so here is not continue, but jump out !!


                    }
//principle 1: if the read and write compete for the same channel source, we give the read priority, so we place the  stop-read possible chip at last.
                    if( (ssd->channel_head[channel].chip_head[temp_chip_number].chip_haveto_find_a_new_write == 1) && (ssd->channel_head[channel].gc_command == NULL ) )  {

            printf("now there is no GC in channel %d ,chip %d  and need to find a new write here.\n", channel, temp_chip_number);
                        continue;
                    }
        ...
        if(find_subread == 1){
//            printf("here go out of r_wait funciton\n");
        break;
        ...
        //
        //                    //here is not jumped 
        //               if( (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_read_real_stop == 1) && (ssd->channel_head[channel].gc_command == NULL ) ){ 
        //                       //||( (ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_read_real_stop == 1) && (ssd->channel_head[channel].gc_command != NULL ) &&() )  ) {
        //                   break;
        //               }
        //
        //
        ...
        //        }
        //    }
}//end for if no advanced commands 
return SUCCESS;
}
...
int judge_write_existing_in_chip(struct ssd_info *ssd, int channel, int chip)// return 1 means write exit in this chip
{
    ...
    int write_number = 0;
    struct sub_request *temp_sub = NULL;
    temp_sub = ssd->channel_head[channel].subs_w_head;
    while(temp_sub != NULL){
...
//judge whether read sub_request exist in pointed chip

int judge_2read_existing_in_chip(struct ssd_info *ssd, int channel, int chip)// return 1 means write exit in this chip
{
...
#ifdef DEBUGADJUSTWRITE 
    printf("deletewritesub :channel [%d], chip [%d],die [%d], block[%d], page[%d], write queue length minus one %d, and sub is %lld\n",sub->location->channel, sub->location->chip, sub->location->die,  sub->location->block,  sub->location->page, ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].chip_write_queue_length, sub);
#endif
...
#endif
    unsigned int has_find_sub = 0;

    //so many steps ,but only here make sence,since there is no advance cmd design here.
//
//    ///***********************************add principle for the choose for writesub******************//
////step1: in this channel, we just find the which chip has chip_for_write_sub sign, i
////step2:if there is so many chips in one channel, we use writetoken  (chip_writetoken_adjusting)  for loop. 
////step3:if there is no any one chip has chip_for_write_sub sign, we use another writetoken (chip_writetoken)to represent the sequence.
//    unsigned int chip_begin_for_adjusting = ssd->channel_head[channel].chip_writetoken_adjusting + 1;
//
//    unsigned int chip_number_setting = ssd->channel_head[channel].chip_writetoken_adjusting +ssd->channel_head[channel].chip;
//    unsigned int temp1 = 0;
//
//
//    ///***********************************add principle for the choose for writesub******************//
//
//            for (chip = chip_begin_for_adjusting; chip < ssd->channel_head[channel].chip; chip++) {
//
//
//// do modle value
//if(temp1 > ((ssd->parameter->chip_num / ssd->parameter->channel_number)-1)){
...
//
                                ...
                                while (sub != NULL) {
#ifdef  DEBUGADJUSTWRITE            
                                    printf("we list all writesub in channel %d, sub is %lld\n", channel, sub);
                                        ...
                                        if ((ssd->channel_head[channel].chip_head[chip].current_state == CHIP_IDLE) ||
                                            ...
                                            if (*channel_busy_flag == 0) {
                                                        ...
                                                        ((ssd->parameter->advanced_commands & AD_TWOPLANE) != AD_TWOPLANE))     /*不执行高级命令*/
                                                {
                                                    ...
                                                    //if we want to use other scheduler, we should add this sentence here to mask upside result which depends one one one big request.
                                                    //you know, only too much write request, then there is chip and channel IDLE chance for write, so ,if there is onechance, we certainly want to service the recent and only one  write request's here. so, the same request's subrequest was designed together.so choose from one request is the best choice. so we can find that read for this scheduler is also the best one .. 

                                                    //sub = after_scheduler_to_get_writesubrequest( ssd, channel, chip); // but here is upside is already the request's subrequests. so if there are other, such as slacktime scheduler will be useful.
...
#ifdef DEBUGADJUSTWRITE 
                                                            printf("write begin!!now begin run the go_one_step, sr_w_c_a_data_transfer, serviced in now %d channel, %d chip,   sub is %lld, write_preemptive_sign is %d\n",channel, chip, sub, ssd->channel_head[channel].chip_head[chip].write_preemptive_sign);
#endif

                                                            ssd->channel_head[sub->location->channel].chip_head[sub->location->chip].assigned_willbe_dealed_write_sub = sub;// here is the only way to assign write subrequst for this chip
                                ...
                                }//end of while for subs_w_head loop
        }//end else statistic strategy
    }//end of subs_w_head is no NULL
    return SUCCESS;
        ...
        for (chip = 0; chip < ssd->channel_head[channel].chip; chip ++) {
            if(ssd->channel_head[channel].chip_head[chip].write_preemptive_sign == 1){
                ...
                while(sub_for_write != NULL)
                {
                    ...
                    printf("no read ,so have to normal restore write, now there is no read, so, have to try to restore write from channel %d, chip %d, stored sub is %lld, num_write_request is %d\n",  channel, chip, ssd->channel_head[channel].chip_head[chip].storedsub, ssd->channel_head[channel].chip_head[chip].num_w_cycle);
#endif
...
#ifdef DEBUGSUSPEND
                    printf("this chip has no read now, so begin to restore write,but have to judgment channel is IDLE and the num_w_cycle is %d.\n",ssd->channel_head[channel].chip_head[chip].num_w_cycle);
#endif
            ...
            printf("this chip has no read now, so begin to restore write,suspension detailed information. chip: %d, lpn: %d, ppn: %d  operation: %d,size: %d,   ////sub_request status:: current_state: %d, current_time: %llu, next_state: %d, next_state_predict_time: %llu, state(only for write): %d\n", temp_sub_for_store->location->chip, temp_sub_for_store->lpn, temp_sub_for_store->ppn,temp_sub_for_store->operation, temp_sub_for_store->size, temp_sub_for_store->current_state,temp_sub_for_store->current_time,temp_sub_for_store->next_state, temp_sub_for_store->next_state_predict_time, temp_sub_for_store->state) ;
...
#ifdef DEBUGADJUSTWRITE 
                printf("there is no suspended write now in channel %d, chip %d\n", channel, chip);
#endif
            }
...
int try_to_restore_write_function_during_adjust_write_iterations(struct ssd_info *ssd, int channel, int chip){
...
//        for (chip = 0; chip < ssd->channel_head[channel].chip; chip ++) {
            ...
            if(ssd->channel_head[channel].chip_head[chip].write_preemptive_sign == 1){
#ifdef DEBUGADJUSTWRITE 
                printf("adjust_write_try to restore already write suspended channel [%d], chip [%d] .\n", channel, chip);
#endif
                // actually, here dont have to judge the 
...
//                if(sub_for_read == NULL){
                ...
                while(sub_for_write != NULL)
                {
                    ...
                    temp_sub_for_store = ssd->channel_head[channel].chip_head[chip].storedsub;
            //printf("restore the write information is : channel %d, chip: %d, sub %lld, lpn: %d, ppn: %d  operation: %d,size: %d,   ////sub_request status:: current_state: %d, current_time: %llu, next_state: %d, next_state_predict_time: %llu, state(only for write): %d\n", temp_sub_for_store->location->chip,temp_sub_for_store, temp_sub_for_store->lpn, temp_sub_for_store->ppn,temp_sub_for_store->operation, temp_sub_for_store->size, temp_sub_for_store->current_state,temp_sub_for_store->current_time,temp_sub_for_store->next_state, temp_sub_for_store->next_state_predict_time, temp_sub_for_store->state) ;
                    if(sub_for_write != temp_sub_for_store){
                        printf("lxclxclxc,so serious wrong!!!, chip %d first write %lld\n", chip, sub_for_write);
                        while(1){}
                    }
//#ifdef DEBUGSUSPEND
                    printf("this chip has read but need change priority, judgment channel is IDLE and the num_w_cycle is %d.\n",ssd->channel_head[channel].chip_head[chip].num_w_cycle);
//#endif
                    //write 恢复的时候，按照原来存储的下个状态，将下一阶段需要消耗的时间存储在chip中，还有要存储的是：下个要开始的阶段。以及下个开始阶段的用时。由于write比较固定，因此只用存下次要开始的次数就行。而gc部分，由于很复杂，所以要存储下一阶段标记以及下阶段的用时。但是你不能设定chip的当前时间！！！！并且write不用存储当前状态，因为write中间都设定成同一个状态了。
                    ...
                    //if ((ssd->channel_head[channel].current_state == CHANNEL_IDLE) || (ssd->channel_head[channel].next_state == CHANNEL_IDLE && ssd->channel_head[channel].next_state_predict_time <= ssd->current_time)) {
                        ...
                        printf("now begin to restore,channel %d, chip %d, sub %lld, num_w_cycle %d\n", channel, chip, sub_for_write, ssd->channel_head[channel].chip_head[chip].num_w_cycle);
#ifdef DEBUGSUSPEND
                        printf("now really can restore .\n");
#endif
...
//                        // ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;// 
                        ...
                        if( (ssd->channel_head[channel].chip_head[chip].num_w_cycle) % 2 == 1 ){
                            ...
                            ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->current_time + ssd->parameter->time_characteristics.tPROG_w_progam ;// this can avoid a new write begin.
                        }else {
                            ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->current_time + ssd->parameter->time_characteristics.tPROG_w_verify;// this can avoid a new write begin.
                        }
...
//                }//end of channel IDLE
                            ...
                            printf("now channel %d, chip %d, has no suspension write now\n",channel, chip);
                        }
...
#ifdef DEBUGADJUSTWRITE 
                    printf("before go into new_services_2_write,channel is %d\n",i);
#endif 
                    new_services_2_write(ssd, i, &flag, &chg_cur_time_flag);
                    //}
#ifdef DEBUGADJUSTWRITE 
                    printf("finish new_services_2_write, flag is %d, flag_gc is %d\n",flag, flag_gc);
#endif
#ifdef DEBUG 
...
#ifdef DEBUGADJUSTWRITE             
                printf("finish go_one)step SR_R_C_A_TRANSFER: sub information: channel %d, chip %d, die %d, plane %d, ppn %d, sub->current_time %lld, sub->next_time %lld, ssd->current_time %lld, ssd->current_state %d, ssd->next_state %d, chip %d  next_predict_time %lld, sub->ppn = ssd.add_reg_ppn is %d  \n",sub->location->channel, sub->location->chip, sub->location->die, sub->location->plane, sub->ppn,  sub->current_time, sub->next_state_predict_time, ssd->current_time,sub->current_state, sub->next_state,location->chip,      ssd->channel_head[location->channel].chip_head[location->chip].next_state_predict_time, sub->ppn );
#endif
