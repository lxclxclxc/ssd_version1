...
// add for erase iterations
    ssd->erase_iterations = ERASE_ITERATIONS;
...
struct plane_info *initialize_plane(struct plane_info *p_plane, struct parameter_value *parameter) {
    ...
    //lxcprogram
   // p_plane->num_w_cycle = 0;
   // p_plane->write_preemptive_sign = -1; // for write add 1 eveytime.
   // p_plane->pos_in_iteration = -1; // 0 means program, 1 means verify.

   // p_plane->backup_buffer_register_ppn = -1;// backup for preemptived write page.
...
struct chip_info *initialize_chip(struct chip_info *p_chip, struct parameter_value *parameter, long long current_time) {
    ...
    p_chip->assigned_willbe_dealed_write_sub = NULL;//this is designed specially for write subrequest. for further scheduler. 
    p_chip->assigned_willbe_dealed_read_sub = NULL;//this is designed specially for read subrequest. for further scheduler. 
    //lxcprogram
    p_chip->num_w_cycle = 0;
    p_chip->write_preemptive_sign = 0; // for write add 1 eveytime.
    p_chip->chip_read_queue_length = 0;
    p_chip->chip_write_queue_length = 0;
    p_chip->chip_for_write_sub = 0;
...
struct ssd_info *initialize_channels(struct ssd_info *ssd) {
    ...
    for (i = 0; i < ssd->parameter->channel_number; i++) {
        ...
        p_channel->chip_readtoken = 0;
        p_channel->chip_writetoken = 0;
        p_channel->channel_for_write_sub = 0;
        //lxcprogram_gc
        //p_channel->gc_channel_sign = 0;
        //ssd->channel_head[i].erase_sign = 0;
...
#ifdef DEBUG
            printf("initialize channels chip %d, ssd current_time %lld\n",j, ssd->current_time);
#endif
        }
    }
...
struct parameter_value *load_parameters(char parameter_file[30]) {
    ...
    p->queue_length = 100; //lxcprogram_gc  original wrong!!!here.why?????
    ...
    while (fgets(buf, 200, fp)) {
        ...
        pre_eql = ptr - buf;//此处buf是整个fgets的起始地址，因为开头，虽然不一定是0 lxc
        next_eql = pre_eql + 1;

        while (buf[pre_eql - 1] == ' ') pre_eql--;
        buf[pre_eql] = 0;// 该地方没有意义吧？有，为了截取buf内容。
        ...
        //lxcprogram
        else if ((res_eql = strcmp(buf, "tPROG_r_phy")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_r_phy) ;
        } else if ((res_eql = strcmp(buf, "tPROG_w_total ")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_w_total) ;
        } else if ((res_eql = strcmp(buf, "tPROG_Nw_cycle")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_Nw_cycle) ;
        } else if ((res_eql = strcmp(buf, "tPROG_Tw_cycle")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_Tw_cycle) ;
        } else if ((res_eql = strcmp(buf, "tPROG_w_progam")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_w_progam) ;
        } else if ((res_eql = strcmp(buf, "tPROG_w_verify")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_w_verify) ;
        } else if ((res_eql = strcmp(buf, "tPROG_buffer")) == 0) {
            sscanf(buf + next_eql, "%d", &p->time_characteristics.tPROG_buffer) ;
        }
