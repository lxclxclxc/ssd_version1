...
//lxcprogram
#define CHIP_WRITE_DATA_CMD_BUSY 104
#define CHIP_GC_RW_BUSY 105

#define CHIP_DATA_TRANSFER 107
#define CHIP_WAIT 108
#define CHIP_ERASE_BUSY 109  //chip now only has one status sign, not enough.
...
#define SR_W_C_A_DATA_TRANSFER 204
#define SR_W_DATA_TRANSFER_ONE_PROG_OF_ITERATIONS 205
#define SR_W_DATA_TRANSFER_ONE_VERIFY_OF_ITERATIONS 207
#define SR_W_TRANSFER 206

//lxcprogram
#define SR_W_DATA_TRANSFER 208  // just means the write iterations process.
...
struct ac_time_characteristics {
    int tPROG;     //program time   coarse partical size
    //lxcprogram
    int tPROG_r_phy;        // here is normal read + read_extra in suspension.but this design will not use average value for we can 
                           //find the accurate time cost.
    int tPROG_w_total;      // Total program time/Nw_cycle. Here is 40us
    int tPROG_Nw_cycle;     // ISPP iterations number. Here is 5
    int tPROG_Tw_cycle;     // one iterations consisting of tPROG_w_program + tPROG_w_verify (32+8)us. 
    int tPROG_w_progam;     // the flash program time in one iterations 
    int tPROG_w_verify;     // verify the program result in flash. acturally it is the same to tERSE_verify 
    int tPROG_buffer;       // the time taken to load the page buffer with data.
...
struct ssd_info {
    //lxcprogram_gc
unsigned int debug_sign;// 0 means no debug, 1 means debug.
unsigned int gc_suspend_times;// statistic for gc suspend times.
unsigned int write_suspend_times;// statistic for write suspend times.
unsigned int gc_suspend_doing;
unsigned int write_suspend_doing;
int original_total_pages_number;
int after_pre_process_pages_number;
int after_pre_processe_and_aged_pages_number;
int last_total_available_pages_number;
int erase_iterations;
int interference_stuffed_length;
int interference_stuffed_length_during_gc;
int64_t temp_lba;
...
struct channel_info {
    ...
    //lxcprogram_gc
   // int gc_channel_sign; // mark for channel cant service write now.
   // int erase_sign;// gc inner erase sign.
//int gc_preemptive_sign; // for the happening of gc

//lxcprogram_for_new_frame
//no useful now
unsigned int chip_readtoken;// for which chip choosed to be the first one for chip loop for read
unsigned int chip_writetoken;// for which chip choosed to be the first one for chip loop for write 
unsigned int chip_writetoken_adjusting;// for which chip choosed to be the first one for chip loop for write 
//no useful now

unsigned int channel_for_write_sub;//sorry!!! this sign has no useful!!!!!for channel has the right to service writesub after seriving read
};
...
struct chip_info {
    ...
    //lxcprogram
int write_preemptive_sign; // 1 means the preemptive process sign.
int num_w_cycle; // number of the iterarions now

//lxcprogram_de-prioritize

int chip_status;
int chip_status_time;
int chip_read_queue_length;
int chip_write_queue_length;

struct sub_request *storedsub;          //suspended write sub value
...
//now no useful here
unsigned int chip_iteration_change_sign;
unsigned int chip_read_real_stop_write_middle_continue_go;
unsigned int chip_read_real_stop_and_only_write_begin;
unsigned int chip_read_stop; 
//now no useful here

//for write priority
unsigned int chip_only_needto_restore_suspended_write;
unsigned int chip_haveto_find_a_new_write;
//sign for effect of  write iteration number
unsigned int chip_change_write_iterations_number;

// chip's sign for this chip has the right to service write subrequest after servicing read 
unsigned int chip_for_write_sub;


unsigned int chip_read_real_stop;//this means real change write iterations number.

//lxcprogram_for_new_frame
struct sub_request *assigned_willbe_dealed_write_sub;//this is designed specially for write subrequest. for further scheduler.
struct sub_request *assigned_willbe_dealed_read_sub;//this is designed specially for read subrequest. for further scheduler.
...
struct plane_info {
    ...
    //lxcprogram
   // int write_preemptive_sign; // 1 means the preemptiveprocess sign.
   // int num_w_cycle; // number of the iterarions now
   // int pos_in_iteration;// 0 means program, 1 means verify.
   // int backup_buffer_register_ppn;// this is the backup for preemptived write page.
...
struct entry {
    ...
    int state;                      //十六进制表示的话是0000-FFFF，每位表示相应的子页是否有效（页映射）。比如在这个页中，0，1号子页有效，2，3无效，这个应该是0x0003. lxc 顺序为：3210顺序，且有效为1，而且此处可以有至少32个子页。。
};
...
struct gc_operation {
	...
	//lxcprogram_gc
	unsigned int copy_page_number;
	unsigned int copy_valid_number;
	unsigned int pos_in_copy;
        unsigned int read;
        unsigned int write;
        unsigned int erase;// erase 1 :erase. 2: verify
        unsigned int victim_block_number;
       // unsigned int victim_block_number_available_sign;
        unsigned int gc_read_inner;
        unsigned int gc_write_inner;
        unsigned int read_write_end;
        unsigned int last_page_in_victim_deal;    

        unsigned int victim_block_freepage_statistic;
        unsigned int chip_status;
        unsigned int chip_next_status;
        unsigned int sign_for_preemptive;
        int chip_next_predict_time_diff;// recording the next predict time diff for restoring.
};
